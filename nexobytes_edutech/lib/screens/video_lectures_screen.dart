...
class _VideoLectureScreenState extends State<VideoLectureScreen> {
  ...
  read() async {
    ...
    final http.Response response = await http.get(
      Uri.parse("https://nexobytes-edutech.herokuapp.com/api/v1/subject/"+widget.subjectId+"/videos/"+widget.chapterId),
                      ...
                      children: <Widget>[
                        Padding(
                          ...
                          child: Column(
                            crossAxisAlignment: CrossAxisAlignment.start,
                            children: <Widget>[
                              Text("There is no video in this chapter as of now",
                        ...
                        Padding(
                          ...
                          child: Column(
                            ...
                            children: <Widget>[
                              ...
                              SizedBox(height: 30),
                              Column(
                                  children: videos.asMap().map((ind, value) => MapEntry(ind,
                                      CourseContent(number: subjectIndex(ind), url: value.url, title: value.title, description: value.description, publish: value.publish,))).values.toList()
                              ),
  ...
  String getTitle (String subject, String chapter) {
    String result = "VIDEOS / "+subject.substring(0, 3).toUpperCase()+" / "+chapter.substring(0, 5).toUpperCase();
    return result;
  }
...
class CourseContent extends StatelessWidget {
  ...
  const CourseContent({Key? key, required this.number, required this.title, required this.url, required this.description, required this.publish})
      : super(key: key);
